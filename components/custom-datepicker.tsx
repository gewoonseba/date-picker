...
export function CustomDatePicker({ onDateChange }: CustomDatePickerProps) {
  ...
  const daysDifference = Math.round((selectedDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24))
  ...
  const animateToTick = (targetTickIndex: number) => {
    ...
    const animate = (currentTime: number) => {
      ...
      const currentTickIndex = Math.round(startTickIndex + (targetTickIndex - startTickIndex) * easeOutCubic)
  ...
  const getTickIndexFromPosition = (position: number, sliderWidth: number) => {
    ...
    const tickIndex = Math.round((clampedPosition / sliderWidth) * (numTicks - 1) + 0.5) - 0.5
    return Math.max(0, Math.min(numTicks - 1, Math.round(tickIndex)))
  }

  // Update selected tick based on drag position
  const updateSelectedTickFromPosition = (position: number, sliderWidth: number) => {
  ...
  return (
    <div style={{ width: `${componentWidth}px` }}>
      ...
      >
        {/* Tick marks */}
        <div className="absolute top-1/2 transform -translate-y-1/2">{generateTicks()}</div>
      </div>
    </div>
  )
}
